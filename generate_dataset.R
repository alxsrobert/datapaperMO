## Generate data for measlesoutbreaker package
if (Sys.info()[["user"]] %in% c("eidearob", "EIDEAROB")) {
  setwd("H:/PhD/Study/Measles_clustering_susc/")
  #Repository with measles data
  data.dir <- "H:/PhD/Study/Input/"
  #Repository for output
  data.out <- "H:/PhD/Study/Output/clusters_USA/figures_comp_epi/no_imports"
}
...
# From Polymod
polymod <- as.data.table(read.csv(file = "Input/Polymod data/social_contact_UK.csv",
...
dt_loca_pop_center <- as.data.table(read.xlsx2(
  file = paste0(data.dir, "US Data dummy/pop_center.xlsx"),
  sheetIndex = 1, stringsAsFactors = FALSE, colClasses = c("character", "character",
                                                           "character","character",
                                                           "numeric", "numeric",
                                                           "numeric", "character")))
...
dt_distance_pop_center[, county1 := dt_loca_pop_center$ID_COUNTY]
...
set.seed(1)
list_county <- unique(dt_distance$county1)
...
r0_state[r0_state>1] <- 0.99
a <- 0.84
b <- 0.06
dt_distance[, nb_commut := pop_county1**a*exp(-distance_km*b)]
dt_distance[distance_km > 100, nb_commut := 0]
...
table(dt_cases[, cluster]) %>% table %>% print
...
save(fake_outbreak, 
     file ="H:/PhD/Study/Measles_clustering_susc/measlesoutbreaker/data/fake_outbreak.RData")
